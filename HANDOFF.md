...
**What changed**
- `public/lectura/story-3/p0.png`–`p5.png` are Brand CLEAR stills (1152×864 RGB PNG). Byte-identical to the attached pack. p3 REGEN: unmasked Rey Tigre is short dark hair (was long wavy). p0/p1/p2/p4/p5 kept. CAST.md + MANIFEST.md sit with the stills.
- App.jsx `story-3` *El hijo del Rey Tigre* · lucha libre live copy unchanged. George words-vs-stills PASS. Soft chrome parked.
- Content test locks `clearedStory3` bytes/md5/1152×864 and the current words. Path still `${BASE_URL}lectura/story-3/pN.png`.
- Story-1 Brand stills already match the locked md5s on main. No re-lock.
...
- Teach beat: Literal then Why (George one-liner hooks). Next chip. Wrong = shake, chip returns, no bird, no lecture.
- Round clear: short ¡Eso! / That's it. fly-by on the same Cenzontle rig. No second bird.
...
**What changed**
- Safe/Risky item `Quedo a sus órdenes.` Literal is hard gloss only: ES `Quedo bajo sus órdenes.` · EN `I remain under your orders.`
- Soft EN Literal `I’m at your service.` (and `Quedo a su disposición.`) killed for this item.
- Why stays: EN `A Mexican professional close: warm, clear, safe. Fits an email to a client.` · ES `Cierre profesional mexicano: amable, claro, seguro. Encaja en correo con clientas.`
- Soft chrome parked. Do not merge.

**Why**
- Dave/George lock: Literal/Traducción = hard gloss. Soft English / register teach = Why (second beat).
...
**What changed**
- Streak-4 Hoy `¡Eso!` / `That's it.` CONTINUE plays a Yucatán glow beat with **`Abierto` / `Open` only** (same stamps as Bajío / CDMX / Oaxaca). Placement is **before close or idle**, not before the soft paywall.
- CONTINUE uses earned streak (`yucatanUnlockFlashStreak`: yesterday → 4) so raw streak 3 cannot skip. Stale Oaxaca due cannot steal the beat once Oaxaca was seen. `yucatanUnlockSeen` waits until the overlay is on screen (map pin opens after glow). Pin id stays `yucatan`; stamps stay `Yucatán`.
- Once-only. No pep, no new copy, no backpack, no enroll/IAP. Door / paywall / A2HS / Bajío streak-1 → glow → wall / CDMX day-2 → glow before idle / Oaxaca streak-3 → glow before idle stay.

**Why**
- Same class as Oaxaca streak-3 (PR 86): return map beat after earned streak, glow before idle, do not flip Open before the overlay.
...
- Hoy scene path is included (`todaySceneId` / `_today:` — Landlord WhatsApp). Eso done screen arms the beat. Remount still shows the glow before the wall. Copy stays `Abierto` / `Open` only. Once-only. No new pep.
...
**What changed**
- First-session Doctora CONTINUE after `¡Eso!` / `That's it.` lands on a come-back card only (`sessionClose`): existing streak chip + locked teaser `Vuelve mañana por «{title}».` / `Come back tomorrow for “{title}”.` + George dismiss `Listo` / `Done` (not Cerrar/Close/Continuar/Ready).
