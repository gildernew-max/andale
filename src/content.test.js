...
import { SUBJ_FIVE, SUBJ_FIVE_HUB, SUBJ_FIVE_LABEL, SUBJ_FIVE_SUB } from "./subjFive.js";
...
assert(appSrc.includes("data-testid=\"learn-hub\""), "cold Learn open is the fun hub, not a pitch card");
...
assert(/<\/div>\s*\)\}\s*\{showLine && \(\s*<p data-testid="come-back-tomorrow"/.test(appSrc), "titled teaser sits outside first-door-hero — not a CTA");
...
assert(appSrc.includes("hub-phrase-doctor"), "Phrase Doctor is a hub tile");
...
assert(appSrc.includes("hoyStoryForScene"), "Hoy story beat uses the shared story gate");
...
const handoffChunk = appSrc.slice(appSrc.indexOf("post-dismiss-handoff"), appSrc.indexOf("data-testid=\"learn-hub-energy\""));
...
assert(UI.es.learnTagline === "HOME HUB" && UI.en.learnTagline === "HOME HUB", "house wordmark is HOME HUB");
assert(UI.es.vamos === "¡Vamos!" && UI.en.vamos === "¡Vamos!", "Hoy CTA keeps ¡Vamos!");
assert(UI.es.hoyPracticeLine === "Una escena rápida. Mucha diferencia.", "ES Hoy hero line");
assert(UI.en.hoyPracticeLine === "A quick scene. A real difference.", "EN Hoy hero line");
assert(UI.es.hubExplora === "EXPLORA" && UI.en.hubExplora === "EXPLORE", "EXPLORA / EXPLORE");
assert(UI.es.hubMore === "MÁS ACTIVIDADES" && UI.en.hubMore === "MORE ACTIVITIES", "MÁS ACTIVIDADES / MORE ACTIVITIES");
assert(UI.es.hubStories === "Stories" && UI.en.hubStories === "Stories", "Stories is one label");
assert(UI.es.hubGames === "Games" && UI.en.hubGames === "Games", "Games is one label");
assert(UI.es.hubDoctor === "Phrase Doctor" && UI.en.hubDoctor === "Phrase Doctor", "Phrase Doctor is one label");
assert(UI.es.hubEighty === "80/20" && UI.en.hubEighty === "80/20", "80/20 is one label");
assert(UI.es.hubPins === "Pin chase" && UI.en.hubPins === "Pin chase", "Pin chase is one label");
assert(UI.es.hubFlash === "Flashcards" && UI.en.hubFlash === "Flashcards", "Flashcards is one label");
assert(!Object.hasOwn(UI.es, "hubStoriesSub") && !Object.hasOwn(UI.en, "hubStoriesSub"), "no Stories slash tail");
assert(!Object.hasOwn(UI.es, "hubGamesSub") && !Object.hasOwn(UI.en, "hubGamesSub"), "no Games slash tail");
assert(!Object.hasOwn(UI.es, "hubDoctorSub") && !Object.hasOwn(UI.en, "hubDoctorSub"), "no Phrase Doctor slash tail");
assert(!Object.hasOwn(UI.es, "hubEightySub") && !Object.hasOwn(UI.en, "hubEightySub"), "no 80/20 slash tail");
assert(!Object.hasOwn(UI.es, "hubPinsSub") && !Object.hasOwn(UI.en, "hubPinsSub"), "no Pin chase slash tail");
assert(!Object.hasOwn(UI.es, "hubFlashSub") && !Object.hasOwn(UI.en, "hubFlashSub"), "no Flashcards slash tail");
assert(!/hoy-story-chip/.test(appSrc), "featured Hoy has no story chips");
assert(!appSrc.includes("data-testid=\"luna-greeting\""), "coach-bubble header is off the Learn hub");
assert(!appSrc.includes("data-testid=\"camino-more\""), "Más/More is off the Learn hub");
assert(!appSrc.includes("data-testid=\"path-entry\""), "EMPIEZA path-entry is off the Learn hub");
assert(UI.es.more === "Más" && UI.en.more === "More", "L.more key stays for other chrome");
...
assert(appSrc.includes("data-testid=\"learn-hub\""), "Learn home is the fun hub");
assert(appSrc.includes("data-testid=\"learn-tagline\""), "Learn header house wordmark is testable");
assert(appSrc.includes("{L.learnTagline}"), "HOME HUB wordmark follows uiLang");
assert(appSrc.includes("data-testid=\"learn-hub-explora\""), "EXPLORA section is labeled");
assert(appSrc.includes("data-testid=\"learn-hub-energy\""), "Stories + Games are the co-energy row");
assert(appSrc.includes("data-testid=\"learn-hub-more\""), "MÁS ACTIVIDADES section is labeled");
assert(appSrc.includes("data-testid=\"learn-hub-tiles\""), "quiet hub tiles sit under energy row");
assert(appSrc.includes("tier === \"energy\""), "Stories/Games share energy height, quieter than Hoy");
assert(appSrc.includes("repeat(4, 1fr)"), "MÁS ACTIVIDADES is one equal four-tile row");
assert(appSrc.includes("{L.vamos}"), "Hoy CTA includes ¡Vamos!");
assert(appSrc.includes("minHeight: 176"), "Hoy hero stays taller than co-energy tiles");
assert(appSrc.includes("height: energy ? 96 : 78"), "co-energy tiles share one height; quiet row is smaller");
...
assert(appSrc.includes("data-testid=\"hoy-card\""), "Featured Hoy is the Learn hero");
assert(appSrc.includes("hero-cta"), "featured Hoy CTA is testable");
assert(appSrc.includes("come-back-tomorrow"), "home line after win is wired");
assert(appSrc.includes("data-testid=\"learn-path\""), "unit path sits under the hub, not as first chrome");
assert(!/data-testid="camino-more"/.test(appSrc), "Más/More is gone from Learn");
...
assert(!appSrc.includes("camino-daily-workout"), "Camino daily-workout CTA is off the Learn hub");
assert(UI.es.workoutDone === "Rutina hecha", "workoutDone ES key stays");
assert(UI.en.workoutDone === "Routine done", "workoutDone EN key stays");
...
assert(appSrc.includes("eighty-twenty-cta"), "80/20 CTA is testable");
...
assert(SUBJ_FIVE_HUB === "80/20", "hub 80/20 title is the 80/20 loan");
assert(appSrc.includes("L.hubEighty"), "80/20 hub tile title uses L.hubEighty");
assert(!/hub(Stories|Games|Doctor|Eighty|Pins|Flash)Sub/.test(appSrc), "hub tiles have no slash-tail keys");
assert(appSrc.includes("{subjFiveLines(uiLang).map"), "sheet lines follow uiLang");
const eightyRowAt = appSrc.indexOf("learn-hub-tiles");
const exploraAt = appSrc.indexOf("learn-hub-energy");
const hoyHeroAt = appSrc.indexOf('data-testid="hoy-card"');
const pathCardsAt = appSrc.indexOf("let g = -1; // global node index");
assert(hoyHeroAt > 0 && exploraAt > hoyHeroAt && eightyRowAt > exploraAt, "Featured Hoy is louder than EXPLORA and the quiet row");
assert(pathCardsAt > eightyRowAt, "80/20 hub tile sits before path cards");
