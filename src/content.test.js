...
assert(paywallLayout.indexOf("<PaywallFlyAway") < paywallLayout.indexOf("soft-paywall-headline"), "one Cenzontle sits above the title");
...
assert(/<PaywallFlyAway\s*\/>/.test(paywallLayout), "paywall mounts Cenzontle fly-away on the gate");
assert(!/<LogoMark size=\{44\} data-testid="soft-paywall-cenzontle"/.test(paywallLayout), "static LogoMark perch is gone from the gate");
assert((paywallLayout.match(/<LogoMark/g) || []).length === 0, "paywall has no LogoMark perch — fly-away is the one bird");
assert((paywallLayout.match(/<PaywallFlyAway/g) || []).length === 1, "paywall has one fly-away — no second bird");
...
assert(appSrc.includes("from \"./waitlist.js\""), "waitlist helper is wired");
assert(appSrc.includes("from \"./WaitlistStrip.jsx\""), "quiet strip component is wired");
assert(appSrc.includes("shouldShowFreePathWaitlist"), "free-path survival gate is wired");
assert(appSrc.includes("emitFunnelEvent({ event: FUNNEL_EVENTS.waitlistSubmit })"), "waitlist_submit fires with event only");
assert(appSrc.includes("saveWaitlistNotice(waitlistDraft)"), "address stays in the local notice store");
const waitlistSubmitFn = appSrc.slice(appSrc.indexOf("const submitSoftPaywallWaitlist"), appSrc.indexOf("const buySoftPaywall"));
assert(/emitFunnelEvent\(\{ event: FUNNEL_EVENTS\.waitlistSubmit \}\)/.test(waitlistSubmitFn), "successful submit emits waitlist_submit");
assert(!/emitFunnelEvent\(\{[^}]*email/.test(waitlistSubmitFn), "submit handler does not put email on the funnel call");
assert(!/__andaleFunnelLog/.test(waitlistSubmitFn), "submit handler does not write the log itself");
const waitlistSrc = readFileSync(join(dirname(fileURLToPath(import.meta.url)), "WaitlistStrip.jsx"), "utf8");
assert(waitlistSrc.includes("waitlistPrompt(uiLang)"), "waitlist prompt follows uiLang in the strip");
assert(waitlistSrc.includes("waitlistPlaceholder(uiLang)"), "waitlist placeholder follows uiLang");
assert(waitlistSrc.includes("waitlistCta(uiLang)"), "waitlist CTA follows uiLang");
assert(waitlistSrc.includes("waitlistPrivacy(uiLang)"), "waitlist privacy follows uiLang");
assert(waitlistSrc.includes("waitlistSuccess(uiLang)") && waitlistSrc.includes("waitlistError(uiLang)"), "waitlist success and error follow uiLang");
const hubBeforeWall = appSrc.slice(appSrc.indexOf('data-testid="learn-hub"'), appSrc.indexOf('data-testid="soft-paywall"'));
assert(/shouldShowFreePathWaitlist\(\{[\s\S]*paywallDismissed:\s*postDismissHandoff \|\| !!prog\.paywallSeen[\s\S]*unlockedPrem:\s*!!prog\.unlockedPrem[\s\S]*\}\)\s*&&\s*\(\s*<WaitlistStrip/.test(hubBeforeWall), "hub keeps the strip after Continue free");
assert(paywallLayout.indexOf("soft-paywall-dismiss") < paywallLayout.indexOf("<WaitlistStrip"), "modal preview sits under continue free");
assert(paywallLayout.indexOf("soft-paywall-annual") < paywallLayout.indexOf("<WaitlistStrip"), "modal preview does not outrank annual");
assert((appSrc.match(/<WaitlistStrip\b/g) || []).length === 2, "one modal preview and the same strip on the free path");
assert(/background:\s*HUB_CREAM/.test(waitlistSrc), "waitlist field sits on cream");
assert(/border:\s*"1px solid #C46B3A"/.test(waitlistSrc), "waitlist field has a terracotta hairline");
assert(/border:\s*`1px solid \$\{MARK_INK\}`/.test(waitlistSrc), "waitlist CTA has a sage hairline");
assert(/minHeight:\s*44/.test(waitlistSrc), "waitlist field and CTA are tappable");
assert(waitlistSrc.includes("fontSize: 11"), "waitlist prompt matches Listen Skip / soft secondary weight");
assert(!/duo-btn/.test(waitlistSrc), "waitlist CTA is not a duo-btn");
assert(!/D\.green|#58CC02/.test(waitlistSrc), "waitlist CTA is never filled green");
assert(!/<Btn/.test(waitlistSrc), "waitlist does not use the filled annual Btn");
assert(!/\$\d|\$39|\$6\.99|\$99|\bXP\b|streak|racha|Enroll|enroll|Confetti|LogoMark|PaywallFlyAway/.test(waitlistSrc), "waitlist strip has no price, streak, XP, enroll, or second bird");
assert(!/<a\b|href=/.test(waitlistSrc) || waitlistSrc.includes(WAITLIST_PRIVACY_URL), "privacy link is the ASC URL if shown");
assert(!/privacy\.html/.test(waitlistSrc) || waitlistSrc.includes(WAITLIST_PRIVACY_URL), "no other privacy host on the strip");
