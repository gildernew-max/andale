...
assert(paywallLayout.indexOf("soft-paywall-cenzontle") < paywallLayout.indexOf("soft-paywall-headline"), "one Cenzontle sits above the title");
...
assert(/<LogoMark size=\{44\} data-testid="soft-paywall-cenzontle"/.test(paywallLayout), "one static Cenzontle is on the gate");
assert((paywallLayout.match(/<LogoMark/g) || []).length === 1, "paywall has one Cenzontle mark — no second bird");
...
assert(UI.es.hubSobremesaSell === "Los atajos que se te pegan — para que la plática deje de sentirse tarea.", "ES Sobremesa sell is George stamp");
assert(UI.en.hubSobremesaSell === "The shortcuts that stick — so real talk stops feeling like homework.", "EN Sobremesa sell is George stamp");
...
assert(appSrc.includes('from "./sobremesa.js"'), "Sobremesa words live in the content module");
assert(appSrc.includes('data-testid="sobremesa-cta"'), "Sobremesa Intermedio entry is testable");
assert(appSrc.includes("data-testid=\"sobremesa-sheet\""), "Sobremesa sheet is testable");
assert(appSrc.includes("data-testid=\"sobremesa-face\""), "Sobremesa first face is testable");
assert(appSrc.includes("data-testid=\"sobremesa-five\""), "Sobremesa five is testable");
assert(appSrc.includes("data-testid=\"sobremesa-tips\""), "Sobremesa tips expansion is testable");
...
assert(appSrc.includes("{sobremesaFive(uiLang).map"), "Sobremesa five follows uiLang");
assert(appSrc.includes("{sobremesaTips(uiLang).map"), "Sobremesa tips follow uiLang");
assert(!/data-testid="sobremesa-cta"[\s\S]{0,400}Club/.test(appSrc), "Sobremesa entry is not Club");
assert(!/data-testid="sobremesa-cta"[\s\S]{0,400}80%/.test(appSrc), "entry label is not 80%");
assert(appSrc.includes("{L.hubSobremesa}"), "Sobremesa CTA uses the wrap name stamp");
assert(appSrc.includes("{L.hubSobremesaQuiet}"), "Sobremesa CTA uses the wrap quiet stamp");
...
assert(!appSrc.includes("data-testid=\"memory-start\""), "Memory pairs stays out of this restore");
assert(!appSrc.includes("memory-pairs-start"), "no Memory pairs Games tile");
...
const jeopardySlice = appSrc.slice(appSrc.indexOf("screen === \"jeopardy\""), appSrc.indexOf("{/* ---------- STORY READER"));
