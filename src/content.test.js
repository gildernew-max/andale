...
assert(cerezasWhyQ.explain?.es === "El texto dice que la empresa japonesa ofreció un precio premium — así que «no pagaba bien» no es lo que pasó. Aun así don Adán dijo que no: si vendía toda la cosecha a un solo comprador, dependería de uno solo. Eligió independencia sobre un mejor cheque. Por eso gana: no quería depender de una sola empresa.", "cerezas refused ES Why is George expanded stamp");
assert(cerezasWhyQ.explain?.en === "The text says the Japanese company offered a premium price — so “they didn’t pay well” isn’t what happened. Don Adán still said no: if he sold the whole harvest to one buyer, he’d depend on one buyer. He chose independence over a better check. That’s why the right line is he didn’t want to depend on a single company.", "cerezas refused EN Why is George expanded stamp");
...
assert(!/La oferta era premium\. Se negó por independencia/.test(appSrc), "short cerezas Why is gone");
assert(!/The offer was premium\. He refused for independence/.test(appSrc), "short cerezas EN Why is gone");
