...
assert(story0Src.includes("translate(36vw, -18%)"), "enter starts from off-right inside the viewport walk");
assert(story0Src.includes("translate(-50%, -50%) rotate(4deg)"), "hold starts at +4° over the chip");
assert(story0Src.includes("translate(-50%, -50%) rotate(0deg)"), "hold settles to 0°");
assert(story0Src.includes("translate(calc(-50% - 118px), calc(-50% - 158px))"), "exit is a real up-left arc, not a straight translate");
assert(story0Src.includes("translate(calc(-50% - 240px), calc(-50% - 72px))"), "exit finishes the Cubetas-family arc");
